...
Ptr _print(Ptr *stack) { print(stack[0]); return stack[0]; }
...
jmp_buf ex_buf__;
Ptr lerror(char *m) { printf("%s",m); longjmp(ex_buf__, 42); return NIL; }
...
Ptr runCode(Instr *code) {
    ...
    if (err != 0) {
        printf("ERR"); return NIL;
    }

    while (code[p].op != OP_EXIT && ERR==0) {
        switch (code[p].op) {
            case OP_PUSH: PUSH(code[p].value); break;
            case OP_POP: POP(); break;
            case OP_ADD:
            {
                long x = (long)POP(); x = CHECK_INT(x,"ADD expected number as first argument");
                long y = (long)POP(); y = CHECK_INT(y,"ADD expected number as second argument");
                PUSH(INT2PTR(x+y));
                break;
            }
            case OP_SUB:
            {
                long x = (long)POP(); x = CHECK_INT(x,"SUB expected number as first argument");
                long y = (long)POP(); y = CHECK_INT(y,"SUB expected number as second argument");
                PUSH(INT2PTR(x-y));
                break;
            }
            case OP_CONS:
            ...
            case OP_CAR:
            {
                Ptr e = POP(); e = CHECK_TYPE(e,CONS_TYPE,"car of non-cons");
                PUSH(e->cons.car);
                break;
            }
            case OP_CDR:
            {
                Ptr e = POP(); e = CHECK_TYPE(e,CONS_TYPE,"cdr of non-cons");
                PUSH(e->cons.cdr);
                break;
            }
            case OP_CALL:
...
#define PUSHINT(I) { .op = OP_PUSH, .value = INT2PTR(I) }
#define ADD()      { .op = OP_ADD }
#define EXIT()     { .op = OP_EXIT }
#define CALL(F)    { .op = OP_CALL, .value = intern(F) }
